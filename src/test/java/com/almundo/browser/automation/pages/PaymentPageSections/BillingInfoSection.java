...
public class BillingInfoSection extends PageBaseSetup {
    ...
    WebElement CUIL;
    WebElement SITUACION_FISCAL;
    WebElement TIPO_DE_DOCUMENTO;
    WebElement NOMBRE_O_RAZON_SOCIAL;
    WebElement DOMICILIO;
    WebElement NUMERO;
    WebElement PISO;
    WebElement DEPARTAMENTO;
    WebElement CODIGO_POSTAL;
    ...
    BillingInfoSection(WebDriver driver){
        ...
        if(!driver.findElements(By.id("fiscal_name")).isEmpty()){
            ...
            logger.info("Nombre o Razon Social not requiered.");
    ...
    public BillingInfoSection populateBillingInfo(){
        ...
        if(NOMBRE_O_RAZON_SOCIAL != null){ NOMBRE_O_RAZON_SOCIAL.sendKeys("Nombre o Razon Social");}
        if(SITUACION_FISCAL.isEnabled()){selectSituacionFiscal();}
        if(TIPO_DE_DOCUMENTO.isEnabled()){selectTipoDeDocumento();}
