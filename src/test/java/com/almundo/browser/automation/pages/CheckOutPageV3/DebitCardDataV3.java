...
public class DebitCardDataV3 extends CheckOutPageV3 {
    ...
    public void populateDebitCardData(String paymentData, String container){
        ...
        if(inputDef.isRequired("payments","Numero_de_tarjeta",0)){
            setCardNumber(paymentDataObject.get("card_number").toString(), container);}

        if(inputDef.isRequired("payments","Titular_de_la_tarjeta",0)){
        ...
        if(inputDef.isRequired("payments","credit_card_expiration",0)){
            setMonthCardExpiration(paymentDataObject.get("Fecha_de_vencimiento_mes").toString(), container);
            setYearCardExpiration(paymentDataObject.get("Fecha_de_vencimiento_ano").toString(), container);}
        ...
        if (inputDef.isRequired("payments", "document", 0)) {
            selectDocumentType(paymentDataObject.get("Tipo_de_doc").toString(), container);
            setDocumentNumber(paymentDataObject.get("Numero_de_doc").toString(), container);
        }
    }
}